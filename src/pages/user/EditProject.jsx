...
export default function EditProject() {
  ...
  const [rol, setRol]                 = useState(null) // 'autor' | 'colaborador' | null
  ...
  useEffect(() => {
    api.get(`/proyectos/${id}`)
      .then(r => {
        ...
        if (user?._id === autorId) {
          setRol('autor')
        } else if ((p.colaboradores || []).some(c => (c._id || c) === user?._id)) {
          setRol('colaborador')
        } else {
          setRol(null)
        }
  ...
  const submit = async e => {
    ...
    try {
      ...
      if (rol === 'colaborador') {
        // Solo campos permitidos para colaborador
        const camposColab = ['descripcion', 'tecnologias', 'repositorio', 'enlaceDemo', 'tags', 'nivel']
  ...
  // Si no es autor ni colaborador, no tiene acceso
  if (!rol) return (
