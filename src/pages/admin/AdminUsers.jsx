            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              <thead>
                <tr style={{ background:'var(--surface2)', borderBottom:'1px solid var(--border)' }}>
                  {['Usuario','Carrera','Semestre','Rol','Estado','Proyectos'].map(h => (
                    <th key={h} style={{ textAlign:'left', padding:'10px 14px', fontWeight:600, color:'var(--text-2)', fontSize:12, textTransform:'uppercase', letterSpacing:'0.06em' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {users.map((u, i) => (
                  <tr key={u._id} style={{ borderBottom: i<users.length-1 ? '1px solid var(--border)' : 'none', transition:'background 0.1s' }}
                  ...
                  >
                    <td style={{ padding:'12px 14px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                        ...
                        <div>
                          <p style={{ fontWeight:600, color:'var(--text-1)', margin:'0 0 1px' }}>{u.nombre} {u.apellido}</p>
                    ...
                    <td style={{ padding:'12px 14px', color:'var(--text-2)' }}>{u.carrera||'—'}</td>
                    <td style={{ padding:'12px 14px', color:'var(--text-2)', textAlign:'center' }}>{u.semestre||'—'}</td>
                    <td style={{ padding:'12px 14px' }}>
                      <span style={{ fontSize:11, fontWeight:700, padding:'3px 8px', borderRadius:100, background:rolBg[u.rol]||'var(--surface3)', color:rolColor[u.rol]||'var(--text-2)', textTransform:'uppercase', letterSpacing:'0.05em' }}>
                        {u.rol}
                      </span>
                    </td>
                    <td style={{ padding:'12px 14px' }}>
                      <span style={{ fontSize:11, fontWeight:700, padding:'3px 8px', borderRadius:100, background: u.confirmEmail?'var(--success-l)':'var(--warning-l)', color: u.confirmEmail?'var(--success)':'var(--warning)', textTransform:'uppercase', letterSpacing:'0.05em' }}>
                        {u.confirmEmail ? 'Confirmado' : 'Pendiente'}
                      </span>
                    </td>
                    <td style={{ padding:'12px 14px' }}>
                      <Link to={`/proyectos/estudiante/${u._id}`} style={{ fontSize:12, color:'var(--primary)', fontWeight:500, textDecoration:'none' }}>
                        Ver →
                      </Link>
                    </td>
                  </tr>
